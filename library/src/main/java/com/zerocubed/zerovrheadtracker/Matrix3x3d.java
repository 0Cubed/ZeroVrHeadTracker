...
public class Matrix3x3d {
    ...
    public Matrix3x3d(double m00, double m01, double m02, double m10, double m11, double m12,
    ...
    public void setZero() {
        m[0] = m[1] = m[2] = m[3] = m[4] = m[5] = m[6] = m[7] = m[8] = 0.0D;
    }

    public void setIdentity() {
        m[1] = m[2] = m[3] = m[5] = m[6] = m[7] = 0.0D;
        m[0] = m[4] = m[8] = 1.0D;
    }
    ...
    public static void mult(Matrix3x3d a, Matrix3x3d b, Matrix3x3d result) {
        result.set(a.m[0] * b.m[0] + a.m[1] * b.m[3] + a.m[2] * b.m[6], a.m[0] * b.m[1] + a.m[1]
                * b.m[4] + a.m[2] * b.m[7], a.m[0] * b.m[2] + a.m[1] * b.m[5] + a.m[2] * b.m[8],
                a.m[3] * b.m[0] + a.m[4] * b.m[3] + a.m[5] * b.m[6], a.m[3] * b.m[1] + a.m[4] * b
                        .m[4] + a.m[5] * b.m[7], a.m[3] * b.m[2] + a.m[4] * b.m[5] + a.m[5] * b
                        .m[8], a.m[6] * b.m[0] + a.m[7] * b.m[3] + a.m[8] * b.m[6], a.m[6] * b
                        .m[1] + a.m[7] * b.m[4] + a.m[8] * b.m[7], a.m[6] * b.m[2] + a.m[7] * b
                        .m[5] + a.m[8] * b.m[8]);
    }
    ...
    public double determinant() {
        return get(0, 0) * (get(1, 1) * get(2, 2) - get(2, 1) * get(1, 2)) - get(0, 1) * (get(1,
                0) * get(2, 2) - get(1, 2) * get(2, 0)) + get(0, 2) * (get(1, 0) * get(2, 1) -
                get(1, 1) * get(2, 0));
    }
    ...
    public boolean invert(Matrix3x3d result) {
        ...
        if (d == 0.0D) {
            ...
            result.set((m[4] * m[8] - m[7] * m[5]) * invdet, -(m[1] * m[8] - m[2] * m[7]) *
                    invdet, (m[1] * m[5] - m[2] * m[4]) * invdet, -(m[3] * m[8] - m[5] * m[6]) *
                    invdet, (m[0] * m[8] - m[2] * m[6]) * invdet, -(m[0] * m[5] - m[3] * m[2]) *
                    invdet, (m[3] * m[7] - m[6] * m[4]) * invdet, -(m[0] * m[7] - m[6] * m[1]) *
                    invdet, (m[0] * m[4] - m[3] * m[1]) * invdet);
            return true;
        }
    }

    public double maxNorm() {
        double maxVal = Math.abs(m[0]);

        for (int i = 1; i < m.length; ++i) {
            maxVal = Math.max(maxVal, Math.abs(m[i]));
        }
